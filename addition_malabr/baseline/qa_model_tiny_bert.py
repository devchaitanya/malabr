...
def benchmark_infer(
    payload, iterations=20, pool_size=5, warmup=5, log_file="benchmark_log.txt"
):
    # Warmup (not measured)
    ...
    with ThreadPoolExecutor(max_workers=pool_size) as executor:
        ...
        for _ in range(iterations):
            ...
            futures.append(executor.submit(run_with_timing, payload, start_time))
...
if __name__ == "__main__":
    ...
    for i in range(4):
      ...
      print("########### Result #############")
      print("################################")
      print(f"context size: {size_bytes / 1024:.2f}KB")
      payload = {"question": question, "context": context}
      # result = run_with_timing(payload, start_time=time.perf_counter()) * 1000
      # print(f"Time: {result:.2f} ms")
      iteration = 1
      time_total = 0
      for i in range(iteration):
        time_total += run_with_timing(payload, start_time=time.perf_counter())
      
      result = (time_total / iteration) * 1000
      print(f"Time: {result:.2f} ms")
